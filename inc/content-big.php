...
<?php if( get_page_template_slug( get_the_ID() ) == '' || get_page_template_slug( get_the_ID() ) == 'page-single.php' ): ?>
                <?php if( get_post_meta(get_the_ID(), 'my_checkbox', true) == 'white' ){$class = 'white__bottom';}else{$class= 'card card-big';} ?>
                <article id="post-<?php the_ID(); ?>" class="post col-12" data-href="<?php the_permalink() ?>" data-class="post">
...
<?php elseif( get_page_template_slug( get_the_ID() ) == 'adv.php' && !(is_singular( 'post' ) || is_singular( 'page' ) || is_search() ) ): ?>
    ...
    <a href="<?php echo $link; ?>" target="_blank" id="post-<?php the_ID(); ?>" class="post col-12" data-class="post">
